<?php
    ...
    while($row = mysqli_fetch_array($results)) {
        array_push($cronSymbolArray, $row['symbol']);
    }
    //$cron_symbol = substr($cron_symbol, 0, -3);

    foreach ($cronSymbolArray as $cron_symbol) {
        //$cron_symbol = $_POST['symbol'];
        $sql_query = "SELECT * FROM stockrecord where symbol = '".$cron_symbol."'";
        /*$sql_query = "SELECT * FROM stockrecord WHERE id IN ( SELECT symbolid FROM stockgroupmap WHERE subgroupid IN  (SELECT id from stockgroup where `goupname` = 'Australia') )";
        $results = mysqli_query($conn,$sql_query);*/
        $results = mysqli_query($conn,$sql_query);
        $curl        = curl_init();
        while($row = mysqli_fetch_array($results)) {
            curl_setopt_array($curl, array(
                ...
                CURLOPT_URL => 'http://pipes.yahooapis.com/pipes/pipe.run?_id=3e1b7fc9a1a63ea0772d20ce4573d792&_render=json&symbol=' . $cron_symbol,
            ...
            curl_setopt_array($curl, array(
                CURLOPT_RETURNTRANSFER => 1,
                CURLOPT_URL => 'http://pipes.yahooapis.com/pipes/pipe.run?_id=66fdfbbf40eea28d28bac13047b811b1&_render=json&s=' . $cron_symbol . '&a=0&b=1&c=1998&d=' . $today_month . '&e=' . $today_date . '&f=' . $today_year . '&g=d',
                CURLOPT_USERAGENT => 'Codular Sample cURL Request'
            ));
        ...
        $subject = 'Cron Updated for '.$cron_symbol. ' for '. $today_month .'-'.$today_date .'-' . $today_year;
        $message = 'Cron CURL to fetch Yahoo Data :: ' . 'http://pipes.yahooapis.com/pipes/pipe.run?_id=66fdfbbf40eea28d28bac13047b811b1&_render=json&s=' . $cron_symbol . '&a=0&b=1&c=1998&d=' . $today_month . '&e=' . $today_date . '&f=' . $today_year . '&g=d';
        mail('sguha.cn@gmail.com', $subject, $message);
        curl_close($curl);
    }
?>